...
)
{
  ...
  if (sw$use.TDL) {
    ...
    ## assign TDL variables  moved to read.TDL.R 8/18/2011 10:50AM
    #TDL$ind             <- 1:TDL$n;
  }
  ##details<<
  ## END
  ...
  if (sw$use.Licor) {
    ...
    # 2/9/2012 2:01AM
    if (sum(is.na(TL.merge[,1]))) { # NA records in TDL file, see read.TDL.R for # Where TDL$StartSeqFlag!=0 or TDL$SeqActiveFlag==0
      ...
      p.o <- paste("           ", Licor$time[Licor.ind.not.in.TDL], "\n"); wWw <- write.out(p.o);
        ...
        TDL.ind.match.Licor <- TL.merge[is.finite(TL.merge[,2]),1]; # indices of TDL matching Licor times
        if (sum(is.na(TL.merge[,1]))) { # NA records in TDL file, see read.TDL.R for # Where TDL$StartSeqFlag!=0 or TDL$SeqActiveFlag==0
          p.o <- paste("THIS WARNING SHOULD NEVER APPEAR, time.window.TDL.Licor.interp.R times1", "\n"); wWw <- write.out(p.o);
        }
