...
# x.time     =  val.TDL$time
{
  ...
  for (i.plot in plot.format.list)
  {
    ...
    plot.not.na(x.time, gain.12C,   pch=20, type="l", ylab="ppm", main="gain 12CO2");
    plot.not.na(x.time, gain.13C,   pch=20, type="l", ylab="ppm", main="gain 13CO2");
    plot.not.na(x.time, offset.12C, pch=20, type="l", ylab="ppm", main="offset 12CO2");
    plot.not.na(x.time, offset.13C, pch=20, type="l", ylab="ppm", main="offset 13CO2");

    temp.corrected <-
      f.val.calc.corrected(1, 1, gain.12C, gain.13C, offset.12C, offset.13C)
