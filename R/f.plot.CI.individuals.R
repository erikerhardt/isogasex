...
{
  ##details<<
  ## For every variable, plots the mean and bootstrap CI, after removing NAs with \code{\link{plot.not.na}}.
  for (i.plot in plot.format.list)
  {
    ...
    plot.not.na.val <- plot.not.na(bs.time, bs.y, pch=20, type="p", cex=.1, xlab="summary index", ylab="", main=x.title);
    if (plot.not.na.val) {
