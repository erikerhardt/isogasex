...
)
{
  ...
  if (sum(is.na(y.var)) < length(x.time))
    { plot(x.time, y.var, pch=pch, type=type, cex=cex, xlab=xlab, ylab=ylab, main=main); plot.not.na.val <- 1; }
  else
    { plot(0, 0, pch="x", ylab="no data", main=main); plot.not.na.val <- 0; };

  return( plot.not.na.val );
  ### plot.not.na.val
}
