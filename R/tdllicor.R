...
# DRIVER FUNCTION -------------------------------------------------------------
{
  ...
  VARIABLES <- assign.variables(DATA, path)
  val                             <- VARIABLES$val                          ;
  sw                              <- VARIABLES$sw                           ;
  ...
  process.filename <- "process_info.txt";  # hardcoded in write.out() and write.progress()
  if (!is.na(file.info(process.filename)$size)) {
    file.remove(process.filename);
  }; # delete old process_info file
  wWw <- write.out(p.o.temp); # write log so far
  ...
  TDL   <- read.TDL(TDL.fn, sw); # read TDL file,   reference with TDL$[col.name]

    p.o <- paste("Reading Licor data \n"); wWw <- write.progress(p.o, time.start);
  Licor <- read.Licor(Licor.fn, Licor.TDL.time.offset.seconds, sw); # read Licor file, reference with Licor$[col.name]
  ...
  if (sw$use.TDL  ) {
    ...
    if (sw$print.TDL.missing){
      ...
      if (length(bad.ind) > 0) {
        ...
        p.o <- TDL.missing; wWw <- write.progress(p.o, time.start, type = "matrix");
        rm(TDL.missing);
        p.o <- paste("  Note: Replacing bad TDL values in ", length(bad.ind), "lines with previous value in the file\n"); wWw <- write.progress(p.o, time.start);
        TDL <- fix.missing.TDL.values(TDL, bad.ind);
      }
      rm(bad.ind);
    }
    ...
    if (sw$print.TDL.cycle.timing){
      ...
      extract.cycle.timing(TDL, TDL.cycle, TDL.site.timing.filename); # usually not run because it takes a long time
    }
    ...
    TDL <- last.TDL.index.for.means(TDL, TDL.cycle); # creates TDL$last.list
    ...
    plot.data.cycles(TDL, TDL.cycle, plot.format.list, output.fn.prefix, val$const$Rstd.13C, "full");
    ...
    TDL <- calc.mean.TDL(TDL, TDL.cycle);
    ...
    TDL <- interp.TDL.tanks.ref(TDL, TDL.cycle, plot.format.list, output.fn.prefix, sw)
    ...
    TDL <- calc.mean.TDL.interp.tanks.ref(TDL, TDL.cycle);
  ...
  TDL.Licor.times <- align.TDL.Licor.times(TDL$time, TDL$n, Licor$time, Licor$n, val$timewindow, sw);
  ...
  TDL.and.Licor.interp   <- time.window.TDL.Licor.interp(TDL, Licor, TDL.Licor.times, sw);
  ...
  ## Plot each cycle of TDL measurements (visual diagnostics). \code{\link{plot.data.cycles}}
    ...
    # NOTE, THE lm() does not recognize the value after the ~ if run driver script with source(*.R), but works if copy lines by line.
  plot.data.cycles(TDL, TDL.cycle, plot.format.list, output.fn.prefix, val$const$Rstd.13C, "window");
  ...
  if (sw$use.Licor ) {
    ...
    Licor <- calc.mean.Licor(TDL, Licor, TDL.cycle, sw);
  };
  ...
  val <- val.TDL.Licor.variables(TDL, Licor, TDL.cycle, val, sw);

  ##details<<
  ## summary values (mean). \code{\link{f.val.calc.all.driver}}
    p.o <- paste("Calculate mean values\n"); wWw <- write.progress(p.o, time.start);
  val$calc$sum <- f.val.calc.all.driver(val$sum$TDL, val$sum$Licor, val$const, sw);

  # do val$calc$all "0.1-16" "2012-07-10"
  ##details<<
  ## All time point values. \code{\link{f.val.calc.all.driver}}
    p.o <- paste("Calculate all values\n"); wWw <- write.progress(p.o, time.start);
  val$calc$all <- f.val.calc.all.driver(val$obs$TDL, val$obs$Licor, val$const, sw);
  ...
  if (sw$use.TDL ) {
    ...
    val$write <- write.summary.TDL.file(val, TDL.cycle, output.summary.TDL.fn);
  }
  if (sw$use.Licor ) {
    ##details<<
    ## Licor file. \code{\link{write.summary.Licor.file}}
    val$write <- write.summary.Licor.file(val, TDL.cycle, output.summary.Licor.fn);
  }

  ##details<<
  ## Summary Calculation file. \code{\link{write.summary.Calc.file}}
  val$write <- write.summary.Calc.file(val, TDL.cycle, output.summary.Calc.fn, output.summary.Calc.last.fn);
  ...
  if (sw$write.all.obs.file) {
    ...
    val$write <- write.all.Calc.file(val, TDL.cycle, output.all.Calc.fn);
  }

  ##details<<
  ## plot all the calculated values. \code{\link{plot.val.calc.sum}}
    p.o <- paste("Plot mean values\n"); wWw <- write.progress(p.o, time.start);
  plot.val.calc.sum(val$sum$TDL, val$sum$Licor, val$const, val$calc$sum, plot.format.list, output.fn.prefix, sw);
  ...
  if (R.bootstrap > 0) {
    ...
    ##details<< . \code{\link{f.init.bs.matrix}}
    # init bs values to zero
    val$calc$bs <- f.init.bs.matrix(val$sum$TDL$n, R.bootstrap);

      p.o <- paste("Calculating the ",R.bootstrap," bootstrap iterates\n"); wWw <- write.progress(p.o, time.start);
    for (i.bs in 1:R.bootstrap) {
        if (i.bs == 1 ) { time.bs.01 <- progress.time(time.start); };
        if (i.bs == 21) { time.bs.21 <- progress.time(time.start);
                          p.o <- paste("  **  Average time per BS sample (based on first 20):", sprintf("%4.2f",(time.bs.21-time.bs.01)/20),"s \n"); wWw <- write.out(p.o);
        };
        p.o <- paste(i.bs," "); wWw <- write.out(p.o);
        if ((i.bs %% 20) == 0) {p.o <- paste("\n"); wWw <- write.out(p.o);};

      ##details<< . \code{\link{f.bs.iter.TDL.Licor}}
      # one BS resample of TDL and Licor means
      val.bs.sum <- f.bs.iter.TDL.Licor(val$obs$TDL, val$sum$TDL, val$obs$Licor, val$sum$Licor, var.interp);
      ##details<< . \code{\link{f.bs.save.TDL.Licor}}
      # save BS resample of TDL and Licor means
      val$bs <- f.bs.save.TDL.Licor(val.bs.sum, val$bs, i.bs, R.bootstrap, sw);
      ##details<< . \code{\link{f.val.calc.all.driver}}
      # This function calls all the calc files
      val.calc.bs.temp <- f.val.calc.all.driver(val.bs.sum$TDL, val.bs.sum$Licor, val$const, sw);
      ##details<< . \code{\link{f.val.bs.matrix}}
      # update values for each bs iterate
      val$calc$bs <- f.val.bs.matrix(val$calc$bs, val.calc.bs.temp, i.bs);
    }
      p.o <- paste("\n"); wWw <- write.out(p.o);
    ...
    val.CI <- f.val.bs.CI(val$calc$bs, val$bs$TDL, val$bs$Licor, R.bootstrap, sig.CI, sw);
    ...
    f.plot.CI.individuals.driver(val, R.bootstrap, plot.format.list, output.fn.prefix);
    ...
    if (sw$use.TDL ) {
      ##details<<
      ## _ TDL file
      val$write <- write.CI.TDL.file(val, TDL.cycle, output.CI.TDL.fn);
    }
    if (sw$use.Licor ) {
      ##details<<
      ## _ Licor file. \code{\link{write.CI.Licor.file}}
      val$write <- write.CI.Licor.file(val, TDL.cycle, output.CI.Licor.fn);
    }
    ##details<<
    ## _ Calculation file. \code{\link{write.CI.Calc.file}}
    val$write <- write.CI.Calc.file(val, TDL.cycle, output.CI.Calc.fn, output.CI.Calc.last.fn);
  ...
  ## Move selected plot files into subdirectories. \code{\link{move.plot.files}}
    ...
    move.plot.files(plot.format.list);
