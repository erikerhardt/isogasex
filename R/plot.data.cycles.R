...
)
{
  ...
  for (i.plot in plot.format.list)
  {
    ...
    s.plot.settings.begin.end(output.fn.prefix, plot.filename, plot.mode = "end");
  } # plotting loop
    ...
    for (i.site in 1:3) {
      ...
      plot(TDL$data[,"ConcA"][ind.all],pch=20, ylab="ConcA", xlab="");
        ...
        for (i.ind.vline in 1:(length(ind.vline)-1)) {
          ...
          #line.y <- my.reg.pred(my.reg.fit(temp.y, temp.x), line.x);
  ...
  for (i.plot in plot.format.list)
  {
    ...
    for (i.site in 1:3) {
      ...
      delta.permil <-
        f.val.calc.delta.permil( TDL$data[,"ConcA"][ind.all], TDL$data[,"ConcB"][ind.all], Rstd.13C);
