...
\item \R\ (requires R 2.15.1+)
  \begin{enumerate}
  \item Windows (\url{http://cran.r-project.org/bin/windows/base/})
...
<<ex, echo=TRUE, size=footnotesize, strip.white=FALSE, eval=FALSE>>=
library(tdllicor)
# ?tdllicor
input.fn <- "tdllicor_template4_Name_WT001.xls"
path <- "/Users/username/Analysis/Name_WT001" # Mac
path <- "C:/Dropbox/Analysis/Name_WT001"      # Windows
tdllicor(input.fn, path)
@
